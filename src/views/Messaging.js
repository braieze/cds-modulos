...
window.Views.Messaging = ({ messages, members, userProfile, addData, updateData, deleteData }) => { // Agregar deleteData
    ...
    const [composeForm, setComposeForm] = useState({ to: '', content: '', isPinned: false });
    ...
    const handleSend = () => {
        ...
        addData('messages', { from: userProfile.id, fromName: userProfile.name, to: composeForm.to, content: composeForm.content, isPinned: composeForm.isPinned, date: new Date().toISOString(), readBy: [] });
        setIsComposeOpen(false); setComposeForm({ to: '', content: '', isPinned: false }); window.Utils.notify("Enviado");
    };
        ...
        <div className="space-y-6 fade-in h-[calc(100vh-140px)] flex flex-col">
            ...
            <div className="flex flex-1 gap-6 overflow-hidden">
                <div className={`w-full md:w-1/3 overflow-y-auto pr-2 space-y-2 ${selectedItem ? 'hidden md:block' : 'block'}`}>
                    {myMessages.map(msg => (
                        <div key={msg.id} onClick={()=>setSelectedItem(msg)} className={`p-4 rounded-xl cursor-pointer border transition-all relative ${selectedItem?.id===msg.id ? 'bg-brand-50 border-brand-200' : 'bg-white border-slate-100 hover:border-brand-200'} ${!msg.readBy?.includes(userProfile.id) ? 'border-l-4 border-l-brand-500' : ''}`}>
                            {msg.isPinned && <div className="absolute top-2 right-2 text-orange-500"><Icon name="Bell" size={14}/></div>}
                            <div className="flex justify-between items-start mb-1 pr-6"><span className="text-sm font-bold text-slate-900">{msg.fromName}</span><span className="text-[10px] text-slate-400">{formatDate(msg.date)}</span></div>
                        ...
                        <div className="animate-enter">
                            <div className="flex justify-between items-start mb-6 border-b border-slate-100 pb-4">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {/* Botón Borrar si soy Pastor o es mi mensaje */}
                                    {(userProfile.role === 'Pastor' || selectedItem.from === userProfile.id) && (
                                        <button onClick={()=>handleDelete(selectedItem.id)} className="text-slate-300 hover:text-red-500"><Icon name="Trash"/></button>
                                    )}
                                </div>
                            </div>
            ...
            <Modal isOpen={isComposeOpen} onClose={()=>setIsComposeOpen(false)} title="Nuevo Mensaje">
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-2 bg-orange-50 p-3 rounded-xl border border-orange-100 cursor-pointer" onClick={()=>setComposeForm(p=>({...p, isPinned: !p.isPinned}))}><div className={`w-5 h-5 rounded border flex items-center justify-center ${composeForm.isPinned ? 'bg-orange-500 border-orange-500 text-white' : 'bg-white border-slate-300'}`}>{composeForm.isPinned && <Icon name="Check" size={14}/>}</div><span className="text-sm font-bold text-orange-800">Fijar este mensaje</span></div>
