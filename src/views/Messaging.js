...
window.Views.Messaging = ({ userProfile }) => {
    ...
    const handleSend = async () => {
        ...
        const msgData = {
            ...
            category: composeForm.category, // Guardamos en español: "Oración", "General", etc.
    ...
    const handleCreateGroup = async () => {
        ...
        try {
            ...
            // 2. Crear mensaje inicial para inicializar el chat
            ...
            setActiveTab('groups'); // Ir a la pestaña
            Utils.notify("Grupo creado");
        } catch (e) { console.error(e); }
    };

    const handleDelete = async (id, ownerId) => {
        // Lógica de permisos: Admin borra todo, usuario borra lo suyo
    ...
    const handleBroadcastReply = async () => {
        ...
        const reply = { 
            id: Date.now(), from: userProfile.id, fromName: userProfile.name, 
    ...
    const handleChatReply = async () => {
        ...
        // Si es chat directo, el 'to' debe ser el ID del otro usuario
    ...
    // 1. VISTA DIFUSIÓN (Cards sin espacios blancos extra)
            ...
            {categorizedMessages.broadcasts.map(msg => (
                <div key={msg.id} onClick={() => setSelectedBroadcast(msg)} className="bg-white rounded-2xl shadow-soft border border-slate-100 overflow-hidden cursor-pointer group hover:shadow-xl transition-all hover:-translate-y-1 relative h-auto flex flex-col">
                    ...
                    <div className="p-4 flex-1 flex flex-col gap-2">
                        ...
                        <h3 className="font-extrabold text-lg text-slate-900 leading-tight line-clamp-2 m-0">{msg.content}</h3>
                        <p className="text-sm text-slate-500 line-clamp-2 flex-1 m-0">{msg.body || 'Ver detalles...'}</p>
                        
                        {/* BARRA REACCIONES COMPACTA */}
                        <div className="flex items-center justify-between border-t border-slate-50 pt-2 mt-2">
                            <div className="flex -space-x-1 items-center">
                                ...
                                <button onClick={(e)=>{e.stopPropagation(); handleReaction(msg.id, '❤️')}} className="w-6 h-6 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 hover:text-red-500 ml-1"><Icon name="Plus" size={12}/></button>
                            </div>
                ...
                <div className="flex-1 overflow-y-auto">
                    {chatList.map(chat => (
                        <div key={chat.chatId||chat.id} onClick={()=>setSelectedChat(chat)} className={`p-4 border-b border-slate-100 cursor-pointer hover:bg-white transition-colors flex gap-3 items-center ${selectedChat?.id===chat.id?'bg-white border-l-4 border-l-brand-500':'border-l-4 border-l-transparent'}`}>
                            <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center font-bold text-slate-500">{chat.title.charAt(0)}</div>
                ...
                {selectedChat ? (
                    <>
                        <div className="p-3 bg-white border-b flex justify-between items-center shadow-sm z-10">
                            ...
                            {(isAdmin || selectedChat.msgs[0].from === userProfile.id) && <button onClick={()=>handleDelete(selectedChat.id, selectedChat.msgs[0].from)} className="text-red-400 hover:text-red-600"><Icon name="Trash" size={18}/></button>}
                        </div>
                        ...
                        <div className="flex-1 p-4 overflow-y-auto space-y-3" ref={scrollRef}>
                            {[...selectedChat.msgs].reverse().map(msg => (
                                <div key={msg.id} className={`flex ${msg.from===userProfile.id?'justify-end':'justify-start'}`}>
                                    <div className={`max-w-[80%] p-3 rounded-lg shadow-sm text-sm ${msg.from===userProfile.id?'bg-[#d9fdd3] text-slate-900':'bg-white'}`}>
                        ...
                        <div className="p-3 bg-white flex gap-2 items-center">
                            ...
                            <button onClick={handleChatReply} className="p-3 bg-brand-600 text-white rounded-full shadow-lg"><Icon name="Send" size={18}/></button>
                        </div>
                    </>
        ...
        <div className="space-y-4 fade-in h-[calc(100vh-140px)] flex flex-col font-sans">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="px-2">
                            ...
                            <div className="flex gap-2 mt-4 pt-4 border-t">
                                {['👍','❤️','🙏','🔥'].map(em => (
                                    <button key={em} onClick={()=>handleReaction(selectedBroadcast.id, em)} className={`px-3 py-1 rounded-full border text-sm hover:scale-110 transition-transform ${selectedBroadcast.reactions?.[userProfile.id]===em ? 'bg-brand-50 border-brand-300' : 'bg-white border-slate-200'}`}>
                ...
                <div className="space-y-5 max-h-[75vh] overflow-y-auto pr-1">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <Select label="Destino" value={composeForm.context} onChange={e=>setComposeForm({...composeForm, context:e.target.value})}>
                            <option value="individual">Persona</option><option value="group">Ministerio</option><option value="custom_group">Grupo</option>{isAdmin&&<option value="broadcast">DIFUSIÓN</option>}
                        </Select>
                        {composeForm.context==='broadcast' ? (
                            ...
                            <Select label={composeForm.context==='group'?'Ministerio':(composeForm.context==='custom_group'?'Grupo':'Persona')} value={composeForm.to} onChange={e=>setComposeForm({...composeForm, to:e.target.value})}>
                                ...
                                {(composeForm.context==='group' ? ['Alabanza','Jóvenes','Escuela Bíblica'] : (composeForm.context==='custom_group' ? groups.map(g=>({id:g.id, name:g.name})) : members)).map(o=><option key={o.id||o} value={o.id||o}>{o.name||o}</option>)}
                            </Select>
                        )}
                    </div>
            ...
            <Modal isOpen={isGroupModalOpen} onClose={()=>setIsGroupModalOpen(false)} title="Nuevo Grupo">
                <div className="space-y-4">
                    ...
                    <div className="max-h-40 overflow-y-auto border p-2 rounded">{members.map(m=><div key={m.id} onClick={()=>{const e=newGroupForm.members.includes(m.id);setNewGroupForm({...newGroupForm, members:e?newGroupForm.members.filter(x=>x!==m.id):[...newGroupForm.members,m.id]})}} className={`p-2 cursor-pointer flex justify-between ${newGroupForm.members.includes(m.id)?'bg-brand-50 text-brand-700':''}`}><span>{m.name}</span>{newGroupForm.members.includes(m.id)&&<Icon name="Check" size={14}/>}</div>)}</div>
