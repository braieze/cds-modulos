...
window.Views.Finances = ({ finances, addData, userProfile }) => {
    ...
    // --- 1. HOOKS (Siempre al principio, nunca dentro de if) ---
    ...
    // --- 2. RENDERIZADO CONDICIONAL (Seguridad) ---
    
    // A. Si no es Pastor, fuera.
        ...
        return (
            <div className="h-full flex flex-col items-center justify-center animate-enter">
                <div className="bg-white p-8 rounded-3xl shadow-soft max-w-sm w-full text-center border border-slate-100">
                    ...
                    <p className="text-slate-500 mb-6 text-sm">Ingresa el PIN de seguridad.</p>
                    <div className="space-y-4">
                        ...
                        <Button className="w-full justify-center py-3 mt-4" onClick={() => { if(pinInput==='1234'){setIsLocked(false);setErrorPin(false);}else{setErrorPin(true);setPinInput('');} }}>Desbloquear</Button>
                    </div>
                </div>
            </div>
        );
    ...
    const handleSave = () => {
        ...
        if(form.type === 'Culto') {
            ...
            final = {...final, amount: form.type==='Gasto' ? -Math.abs(amt) : Math.abs(amt), total: form.type==='Gasto' ? -Math.abs(amt) : Math.abs(amt)};
        }
    ...
    return (
        <div className="space-y-8 fade-in pb-24">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="flex items-center gap-3"><h2 className="text-2xl font-extrabold text-slate-800">Mi Billetera</h2><button onClick={()=>setShowBalance(!showBalance)} className="text-slate-400 hover:text-brand-600"><Icon name={showBalance?"Search":"LogOut"} size={20}/></button></div>
                <div className="flex gap-2"><div className="flex bg-slate-100 p-1 rounded-xl"><button onClick={()=>setTabView('dashboard')} className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${tabView==='dashboard'?'bg-white text-slate-800 shadow-sm':'text-slate-500'}`}>Resumen</button><button onClick={()=>setTabView('list')} className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${tabView==='list'?'bg-white text-slate-800 shadow-sm':'text-slate-500'}`}>Actividad</button></div></div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-gradient-to-br from-brand-600 to-brand-800 text-white p-6 rounded-3xl shadow-lg relative overflow-hidden"><div className="absolute -right-4 -top-4 bg-white/10 w-24 h-24 rounded-full blur-2xl"></div><p className="text-brand-100 text-xs font-bold uppercase tracking-widest mb-1">Saldo Total</p><h3 className="text-4xl font-black">{renderAmount(globalBalances.total)}</h3></div>
                <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-soft flex flex-col justify-between"><div className="flex justify-between items-start"><div><p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Efectivo</p><h3 className="text-2xl font-extrabold text-slate-800">{renderAmount(globalBalances.cash)}</h3></div><div className="bg-emerald-50 text-emerald-600 p-3 rounded-xl"><Icon name="DollarSign" size={24}/></div></div></div>
                <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-soft flex flex-col justify-between"><div className="flex justify-between items-start"><div><p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Banco</p><h3 className="text-2xl font-extrabold text-slate-800">{renderAmount(globalBalances.bank)}</h3></div><div className="bg-blue-50 text-blue-600 p-3 rounded-xl"><Icon name="Briefcase" size={24}/></div></div></div>
            </div>

            {tabView === 'dashboard' && (
                <div className="space-y-6 animate-enter">
                    <div className="flex justify-between items-center"><h3 className="font-bold text-lg text-slate-800">Análisis</h3><DateFilter currentDate={currentDate} onChange={setCurrentDate} /></div>
                    {monthlyData.length === 0 ? (
                        <div className="text-center py-12 bg-slate-50 rounded-3xl border border-dashed border-slate-200"><Icon name="Wallet" size={48} className="text-slate-300 mx-auto mb-4"/><p className="text-slate-500 font-medium mb-4">Sin movimientos.</p><button onClick={window.DataLogic.generateDemoFinances} className="text-brand-600 font-bold hover:underline text-sm">Generar datos prueba</button></div>
                    ) : Recharts && (
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            <Card className="lg:col-span-2 h-80"><h4 className="font-bold text-sm text-slate-500 uppercase mb-4">Flujo Diario</h4><ResponsiveContainer width="100%" height="90%"><AreaChart data={chartData.area}><defs><linearGradient id="colorIng" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.1}/><stop offset="95%" stopColor="#10b981" stopOpacity={0}/></linearGradient><linearGradient id="colorEgr" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#ef4444" stopOpacity={0.1}/><stop offset="95%" stopColor="#ef4444" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9"/><XAxis dataKey="day" tick={{fontSize:10}} axisLine={false} tickLine={false} /><Tooltip /><Area type="monotone" dataKey="ingreso" stroke="#10b981" fillOpacity={1} fill="url(#colorIng)" strokeWidth={3} /><Area type="monotone" dataKey="egreso" stroke="#ef4444" fillOpacity={1} fill="url(#colorEgr)" strokeWidth={3} /></AreaChart></ResponsiveContainer></Card>
                            <Card className="h-80 flex flex-col"><h4 className="font-bold text-sm text-slate-500 uppercase mb-4">Gastos</h4><div className="flex-1 relative">{chartData.pie.length > 0 ? (<ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={chartData.pie} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">{chartData.pie.map((entry, index) => <Cell key={`cell-${index}`} fill={PIE_COLORS[index % PIE_COLORS.length]} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer>) : <div className="flex h-full items-center justify-center text-slate-400 text-xs italic">Sin gastos</div>}</div></Card>
                        </div>
                    )}
                </div>
            )}
            ...
            {tabView === 'activity' && (
                <div className="space-y-4 animate-enter">
                    <div className="flex justify-between items-center"><div className="flex items-center gap-2"><DateFilter currentDate={currentDate} onChange={setCurrentDate} /></div><Button icon="Plus" onClick={()=>{setForm({...initialForm, date: Utils.getLocalDate()}); setIsModalOpen(true);}}>Movimiento</Button></div>
                    <div className="bg-white rounded-3xl shadow-soft border border-slate-100 overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm text-left"><thead className="bg-slate-50/50 text-slate-500 font-bold uppercase text-[10px] border-b border-slate-100"><tr><th className="p-4">Fecha</th><th className="p-4">Concepto</th><th className="p-4 text-right">Monto</th><th className="p-4"></th></tr></thead><tbody className="divide-y divide-slate-50">{monthlyData.map(f => (<tr key={f.id} className="hover:bg-slate-50 transition-colors"><td className="p-4 whitespace-nowrap text-slate-500 font-medium text-xs">{formatDate(f.date)}</td><td className="p-4"><div className="font-bold text-slate-800">{f.type==='Culto'?'Cierre Culto':f.category}</div><div className="text-xs text-slate-400">{f.notes}</div></td><td className={`p-4 text-right font-mono font-bold ${(f.total||f.amount)<0?'text-red-600':'text-emerald-600'}`}>{formatCurrency(f.total||f.amount)}</td><td className="p-4 text-right">{f.attachmentUrl && <button onClick={()=>setReceiptModal(f.attachmentUrl)} className="text-slate-300 hover:text-brand-600"><Icon name="Image" size={16}/></button>}</td></tr>))}</tbody></table></div></div>
                </div>
            )}
