...
window.Views.Finances = ({ finances, addData, userProfile }) => {
    ...
    // (Opcional: descomentar si se quiere usar componentes directos, 
    // pero es más seguro usar Recharts.ResponsiveContainer en el JSX)

    // 2. TODOS LOS HOOKS (Estados y Memos)
    // Estado de la UI
    ...
    const categories = [{ value: 'General', label: 'General / Varios' }, { value: 'Mantenimiento', label: 'Infraestructura' }, { value: 'Honorarios', label: 'Honorarios' }, { value: 'Alquiler', label: 'Alquiler' }, { value: 'Ayuda Social', label: 'Ayuda Social' }, { value: 'Ministerios', label: 'Ministerios' }, { value: 'Ofrenda Misionera', label: 'Ofrenda Misionera' }];
    ...
    const chartData = useMemo(() => {
        ...
        finances.forEach(f => {
            ...
            if (trendMap[key]) {
                ...
                if (inc) trendMap[key].ingresos += Math.abs(val); else trendMap[key].gastos += Math.abs(val);
            }
        });
        ...
        monthlyData.filter(f => safeNum(f.total || f.amount) < 0).forEach(f => {
            ...
            pieMap[cat] = (pieMap[cat] || 0) + Math.abs(safeNum(f.total || f.amount));
        });
    ...
    const handleSave = () => {
        ...
        if(form.type === 'Culto') {
            ...
            const amt = safeNum(form.amount); if(amt===0) return Utils.notify("Monto 0", "error");
            const v = Math.abs(amt); f = {...f, amount: form.type==='Gasto'?-v:v, total: form.type==='Gasto'?-v:v};
        }
    ...
    const renderAmount = (amount) => showBalance ? formatCurrency(safeNum(amount)) : "$ ••••••";

    // --- 3. RENDERIZADO CONDICIONAL ---
    // (Ahora que los hooks ya se declararon, podemos hacer returns)
    ...
    if (isLocked) return (
        <div className="h-full flex flex-col items-center justify-center animate-enter"><div className="bg-white p-8 rounded-3xl shadow-soft max-w-sm w-full text-center border border-slate-100"><div className="bg-brand-100 w-16 h-16 rounded-2xl mx-auto flex items-center justify-center text-brand-600 mb-6"><Icon name="Wallet" size={32}/></div><h2 className="text-2xl font-extrabold text-slate-800 mb-2">Billetera</h2><form onSubmit={handleUnlock} className="space-y-4"><input type="password" maxLength="4" className="text-center text-3xl font-bold w-full border-b-2 outline-none py-2" value={pinInput} onChange={e=>setPinInput(e.target.value)} /><Button className="w-full mt-4" onClick={handleUnlock}>Desbloquear</Button></form></div></div>
    );
        ...
        <div className="space-y-8 fade-in pb-24">
            ...
            {tabView==='dashboard' && Recharts && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-enter">
                    ...
                    {finances.length === 0 && <div className="lg:col-span-3 text-center py-4 bg-slate-50 rounded border border-dashed"><button onClick={window.DataLogic.generateDemoFinances} className="text-brand-600 font-bold hover:underline">Generar Datos de Prueba (Oct/Nov)</button></div>}
            ...
            {tabView==='list' && (<div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm text-left"><thead className="bg-slate-50 text-slate-500 font-bold uppercase text-xs"><tr><th className="p-4">Fecha</th><th className="p-4">Detalle</th><th className="p-4 text-right">Monto</th></tr></thead><tbody className="divide-y divide-slate-100">{monthlyData.map(f=>(<tr key={f.id}><td className="p-4 whitespace-nowrap text-slate-600">{formatDate(f.date)}</td><td className="p-4"><div className="font-bold text-slate-800">{f.type==='Culto'?'Cierre':f.category}</div><div className="text-xs text-slate-500">{f.notes||f.method}</div></td><td className={`p-4 text-right font-mono font-bold ${Number(f.total||f.amount)<0?'text-red-600':'text-emerald-600'}`}>{formatCurrency(f.total||f.amount)}</td></tr>))}</tbody></table></div></div>)}
