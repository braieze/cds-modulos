...
window.Views.Finances = ({ finances, addData, userProfile }) => {
    ...
    const chartData = useMemo(() => {
        ...
        for(let i=5; i>=0; i--) { 
            ...
            trendMap[key] = { name: Utils.formatDate(d.toISOString().slice(0,10), 'month'), ingresos: 0, gastos: 0 }; 
        }
        ...
        finances.forEach(f => {
            ...
            // Solo procesar si entra en el rango de 6 meses o es el mes actual
            if (trendMap[key]) {
                let val = 0;
                let isIncome = false;
                if (f.type === 'Culto') {
        ...
        const pieMap = {};
        monthlyData.filter(f => f.type === 'Gasto' || (f.amount && f.amount < 0)).forEach(f => {
            const cat = f.category || 'General';
            pieMap[cat] = (pieMap[cat] || 0) + Math.abs(Number(f.amount || 0));
        });
    ...
    const Recharts = window.Recharts; // Acceso directo en render para evitar undefined
        ...
        <div className="space-y-8 fade-in pb-24">
            ...
            {tabView === 'dashboard' && Recharts && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-enter">
                    <Card className="lg:col-span-2 h-72">
                        ...
                        <div className="h-full w-full -ml-4" style={{ minHeight: '250px' }}>
                            <Recharts.ResponsiveContainer width="100%" height="100%">
                                <Recharts.AreaChart data={chartData.trend}>
                                    <Recharts.defs><linearGradient id="colorIng" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.1}/><stop offset="95%" stopColor="#10b981" stopOpacity={0}/></linearGradient><linearGradient id="colorEgr" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#ef4444" stopOpacity={0.1}/><stop offset="95%" stopColor="#ef4444" stopOpacity={0}/></linearGradient></Recharts.defs>
                    ...
                    <Card className="h-72 flex flex-col">
                        ...
                        <div className="flex-1 relative" style={{ minHeight: '200px' }}>
                            ...
                            ) : <div className="flex h-full items-center justify-center text-slate-400 text-xs italic">Sin gastos registrados</div>}
                        </div>
                    </Card>
                </div>
            )}
            ...
            {finances.length === 0 && (
                <div className="text-center py-8">
                    <button onClick={window.DataLogic.generateDemoFinances} className="text-brand-600 font-bold hover:underline">
