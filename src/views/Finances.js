...
window.Views.Finances = ({ finances, addData, userProfile }) => {
    ...
    // Acceso seguro a gráficos (Si no existe, es null)
    ...
    const chartData = useMemo(() => {
        ...
        const trendMap = {};
        for(let i=5;i>=0;i--){ const d=new Date(); d.setMonth(d.getMonth()-i); const k=d.toISOString().slice(0,7); trendMap[k]={name:k,label:Utils.formatDate(d.toISOString().slice(0,10),'month'),ingresos:0,gastos:0}; }
        finances.forEach(f=>{
        ...
        const pieMap={}; monthlyData.filter(f=>safeNum(f.total||f.amount)<0).forEach(f=>{ const c=f.category||'General'; pieMap[c]=(pieMap[c]||0)+Math.abs(safeNum(f.total||f.amount)); });
    ...
    if (isLocked) return (
        <div className="h-full flex flex-col items-center justify-center animate-enter"><div className="bg-white p-8 rounded-3xl shadow-soft max-w-sm w-full text-center border border-slate-100"><div className="bg-brand-100 w-16 h-16 rounded-2xl mx-auto flex items-center justify-center text-brand-600 mb-6"><Icon name="Wallet" size={32}/></div><h2 className="text-2xl font-extrabold text-slate-800 mb-2">Billetera</h2><input type="password" maxLength="4" className="text-center text-3xl font-bold w-full border-b-2 outline-none py-2" value={pinInput} onChange={e=>setPinInput(e.target.value)} placeholder="••••" /><Button className="w-full mt-4" onClick={()=>{if(pinInput==='1234'){setIsLocked(false);setErrorPin(false)}else{setErrorPin(true);setPinInput('')}}}>Desbloquear</Button>{errorPin && <p className="text-red-500 text-xs mt-2">PIN Incorrecto</p>}</div></div>
    );
        ...
        <div className="space-y-8 fade-in pb-24">
            ...
            {tabView==='dashboard' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-enter">
                    {finances.length===0 && <div className="lg:col-span-3 text-center py-4 bg-slate-50 rounded border border-dashed"><button onClick={window.DataLogic.generateDemoFinances} className="text-brand-600 font-bold hover:underline">Generar Datos de Prueba</button></div>}
                    {Recharts && chartData.trend.length>0 ? (
                        <>
                    ...
                    ) : <div className="col-span-3 text-center p-4 bg-red-50 text-red-500 rounded-xl border border-red-100">Librería gráfica no cargada o sin datos. Intenta recargar.</div>}
                </div>
            )}
