...
window.Views.Finances = ({ finances, addData, userProfile }) => {
    ...
    if (userProfile?.role !== 'Pastor') return <div className="p-8 text-center text-slate-500">Acceso restringido a Pastoral.</div>;
    ...
    // Categorías
    const categories = [{value:'General',label:'General'},{value:'Mantenimiento',label:'Mantenimiento'},{value:'Honorarios',label:'Honorarios'},{value:'Alquiler',label:'Alquiler/Servicios'},{value:'Ayuda Social',label:'Ayuda Social'},{value:'Ministerios',label:'Ministerios'}];
    ...
    const chartData = useMemo(() => {
        ...
        for(let i=5; i>=0; i--) {
            ...
            trendMap[d.toISOString().slice(0,7)] = { name: Utils.formatDate(d, 'month'), ingresos: 0, gastos: 0 };
        }
        finances.forEach(f => {
    ...
    const handleSave = () => {
        ...
        if(form.type === 'Culto') {
            ...
            if(total===0) return alert("Total en 0");
        ...
        addData('finances', final); setIsModalOpen(false); setForm(initialForm);
    };

    // Generar Gastos Fijos Automáticos
    const generateFixedExpenses = () => {
        if(!confirm("¿Generar gastos fijos del mes (Alquiler, Luz, Internet)?")) return;
        const fixed = [
            { type: 'Gasto', category: 'Alquiler', amount: -50000, method: 'Banco', notes: 'Alquiler Templo', date: new Date().toISOString().slice(0,10) },
            { type: 'Gasto', category: 'Mantenimiento', amount: -5000, method: 'Efectivo', notes: 'Limpieza Mensual', date: new Date().toISOString().slice(0,10) }
        ];
        fixed.forEach(f => addData('finances', { ...f, total: f.amount }));
        alert("Gastos generados.");
    };
        ...
        <div className="space-y-8 fade-in pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex gap-2">
                    <div className="flex bg-white p-1 rounded-xl shadow-sm border border-slate-100">
                        <button onClick={()=>setTabView('dashboard')} className={`px-4 py-2 rounded-lg text-xs font-bold ${tabView==='dashboard'?'bg-brand-50 text-brand-600':'text-slate-500'}`}>Dashboard</button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-lg relative overflow-hidden">
                    ...
                    <p className="text-slate-400 text-xs font-bold uppercase">Total Global</p>
            ...
            {tabView === 'dashboard' && Recharts && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <Card className="lg:col-span-2 h-64">
                        <h3 className="font-bold text-slate-800 mb-2">Flujo de Caja</h3>
                        <Recharts.ResponsiveContainer width="100%" height="100%">
                            <Recharts.LineChart data={chartData.trend}>
                                ...
                                <Recharts.Tooltip />
                                <Recharts.Line type="monotone" dataKey="ingresos" stroke="#10b981" strokeWidth={2} dot={false}/>
                                <Recharts.Line type="monotone" dataKey="gastos" stroke="#ef4444" strokeWidth={2} dot={false}/>
                            </Recharts.LineChart>
                        </Recharts.ResponsiveContainer>
                    </Card>
                    <Card className="h-64 flex flex-col items-center justify-center">
                        <h3 className="font-bold text-slate-800 mb-2 w-full text-left">Gastos del Mes</h3>
            ...
            <div className="space-y-4">
                ...
                {tabView === 'list' && (
                    <div className="flex justify-end">
                        <button onClick={generateFixedExpenses} className="text-xs text-brand-600 font-bold hover:underline flex items-center gap-1"><Icon name="Plus" size={12}/> Generar Gastos Fijos (Alquiler/Luz)</button>
                    </div>
                )}
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-50 text-slate-500 font-bold uppercase text-xs"><tr><th className="p-3">Fecha</th><th className="p-3">Detalle</th><th className="p-3 text-right">Monto</th></tr></thead>
                        <tbody className="divide-y divide-slate-100">
                            {monthlyData.length===0 && <tr><td colSpan="3" className="p-4 text-center text-slate-400">Sin movimientos</td></tr>}
                            {monthlyData.map(f => (
                                <tr key={f.id}>
                                    <td className="p-3 whitespace-nowrap">{formatDate(f.date)}</td>
                                    <td className="p-3">
                                        <div className="font-bold">{f.type === 'Culto' ? 'Cierre Culto' : f.category}</div>
                                        <div className="text-xs text-slate-500">{f.notes || f.method}</div>
                                    </td>
                                    <td className={`p-3 text-right font-mono font-bold ${f.total<0?'text-red-600':'text-emerald-600'}`}>{formatCurrency(f.total || f.amount)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            {/* Modal Registro (Simplificado para el ejemplo, usar el completo anterior si se desea) */}
            <Modal isOpen={isModalOpen} onClose={()=>setIsModalOpen(false)} title="Nuevo Movimiento">
                <div className="space-y-4">
                    <Select label="Tipo" value={form.type} onChange={e=>setForm({...form, type:e.target.value})}><option>Culto</option><option>Gasto</option><option>Ingreso</option></Select>
                    <Input type="date" value={form.date} onChange={e=>setForm({...form, date:e.target.value})}/>
                    {form.type === 'Culto' ? (
                        <div className="grid grid-cols-2 gap-2">
                            <Input label="Diezmo Efec." type="number" value={form.tithesCash} onChange={e=>setForm({...form, tithesCash:e.target.value})}/>
                            <Input label="Ofrenda Efec." type="number" value={form.offeringsCash} onChange={e=>setForm({...form, offeringsCash:e.target.value})}/>
                            <Input label="Diezmo Banco" type="number" value={form.tithesTransfer} onChange={e=>setForm({...form, tithesTransfer:e.target.value})}/>
                            <Input label="Ofrenda Banco" type="number" value={form.offeringsTransfer} onChange={e=>setForm({...form, offeringsTransfer:e.target.value})}/>
                            <Input label="Cant. Diezmantes" type="number" value={form.tithersCount} onChange={e=>setForm({...form, tithersCount:e.target.value})}/>
                        </div>
                    ) : (
                        <>
                            <Input label="Monto" type="number" value={form.amount} onChange={e=>setForm({...form, amount:e.target.value})}/>
                            <Select label="Origen" value={form.method} onChange={e=>setForm({...form, method:e.target.value})}><option>Efectivo</option><option>Banco</option></Select>
