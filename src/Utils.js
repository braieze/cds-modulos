...
window.Utils.formatDate = (d, fmt = 'short') => {
    ...
    // Ajuste simple de zona horaria para fechas YYYY-MM-DD
...
// ==========================================
// 3. COMPONENTES DE UI (NUEVO DISEÑO)
// ==========================================
...
window.Utils.Input = ({ label, type="text", value, onChange, placeholder, className="" }) => (
    <div className={className}>
        ...
        <input 
            ...
            className="w-full bg-white border border-slate-300 text-slate-900 font-medium rounded-xl px-4 py-3 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all shadow-input placeholder:text-slate-400"
        />
    </div>
);
...
window.Utils.Select = ({ label, value, onChange, children, className="" }) => (
    <div className={className}>
        ...
        <select 
            value={value}
            onChange={onChange}
            className="w-full bg-white border border-slate-300 text-slate-900 font-medium rounded-xl px-4 py-3 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all shadow-input outline-none appearance-none"
        >
            {children}
        </select>
    </div>
...
// Navegación por Meses (Chips)
window.Utils.MonthNav = ({ currentDate, onMonthChange }) => {
    const { React } = window;
    ...
    return (
        <div className="flex overflow-x-auto gap-2 pb-2 hide-scroll mb-4">
            {monthChips.map((d, i) => {
                ...
                return (
                    <button 
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all border ${isActive ? 'bg-brand-600 text-white border-brand-600 shadow-lg shadow-brand-500/30' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}
                    >
